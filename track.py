...
class Track(object):
	...
	def Turn(self, turn):						# Generate track border points according track angle = turn
	...
	def Draw(self,f):
		...
		for l in range(len(m)):
			...
			if directionX == -1:
				...
				f.LineTo((trackx - trackw + (self.turn*20), trackh),0x000)		
				f.LineTo((trackx + trackw + (self.turn*20), trackh),0xFFFFFF)
			...
			# Change direction for next line to minimise beam movements 
		...
		for l in range(4,-1,-1): 
			f.LineTo( ((self.trackX[l] - ((self.trackY[l]) - 80) /tracksize) + (self.turn*20), self.trackY[l]),0xFFFFFF)
